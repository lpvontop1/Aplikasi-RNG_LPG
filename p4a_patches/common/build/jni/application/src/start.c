...
int main(int argc, char *argv[]) {
  ...
  /* FIX (p4a issue #1844 + Python 3.12): Cegah re-init Python di process yang sama.
   *
   * Saat Activity di-recreate (onDestroy tanpa process exit), SDL thread lama
   * mungkin tidak sempat memanggil Py_FinalizeEx() sebelum exit(). Akibatnya
   * _PyRuntime.imports.inittab tetap non-NULL. Saat Activity baru launch dan
   * SDL_main dipanggil lagi, PyImport_AppendInittab() akan memanggil
   * Py_FatalError() -> abort() -> SIGABRT (di Python 3.12+; di 3.11 hanya
   * silent no-op yang mengakibatkan black screen).
   *
   * Satu-satunya recovery yang aman: exit process agar Android meluncurkan
   * ulang dengan process fresh. Service p4a jalan di process terpisah
   * (:pythonservice/:service_*) jadi tidak ikut mati.
   */
  if (Py_IsInitialized()) {
    LOGP("Python already initialized in this process (Activity recreation "
         "without clean shutdown). Forcing process exit for clean restart.");
    exit(0);
  }
