...
class KivyRecipe(PyProjectRecipe):
    ...
    def get_recipe_env(self, arch, **kwargs):
        ...
        # Relax strict C errors from NDK 28c clang that break Kivy 2.3.x
        # Cython-generated C code (incompatible function pointer types,
        # implicit function declarations, int conversions).
        relax_flags = (
        ...
        env['CFLAGS'] = env.get('CFLAGS', '') + relax_flags
        env['CXXFLAGS'] = env.get('CXXFLAGS', '') + relax_flags
        # Also append to CC so kivy's setup.py picks it up via LDSHARED chain
        if 'CC' in env:
            env['CC'] = env['CC'] + relax_flags
